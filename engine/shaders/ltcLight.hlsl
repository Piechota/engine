...
float4 psMain( VSToPS input ) : SV_TARGET
{
	...
	for ( uint i = 0; i < 4; ++i )
	{
		verticesVS[ i ] = mul( PlaneVertices[ i ], ObjectToView ).xyz;
	}
	...
	float ltcSpecular = LTC_Evaluate_Specular( normal, eyeVector, positionVS, minV, verticesVS, false );
	float ltcDiffuse = LTC_Evaluate_Diffuse( normal, eyeVector, positionVS, verticesVS, false );
